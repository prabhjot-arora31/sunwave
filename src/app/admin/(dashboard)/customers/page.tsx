...
import { Search, Loader2, Eye, Trash2, Plus, ChevronLeft, ChevronRight, AlertCircle, FileText } from "lucide-react";
import ConfirmDialog from "@/components/admin/ConfirmDialog";

type CustomerRow = Customer & { _count: { documents: number } };
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            {pagination ? `${pagination.total} saved customers` : "Loading..."} - includes walk-ins with no matching lead.
          </p>
        </div>
        <Link
              ...
              {loading ? (
                ...
                customers.map((customer) => (
                  <tr key={customer.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60">
                    <td className="px-5 py-3.5 font-semibold text-slate-900">{customer.fullName}</td>
                    <td className="px-5 py-3.5 text-slate-600">{customer.phone}</td>
                    <td className="px-5 py-3.5 text-slate-600">
                      {customer.leadId ? (
                        <span className="inline-flex items-center text-xs font-semibold rounded-full px-2.5 py-1 bg-sky-100 text-sky-700">
                          From Lead
                        </span>
                      ) : (
                        <span className="inline-flex items-center text-xs font-semibold rounded-full px-2.5 py-1 bg-slate-100 text-slate-600">
                          Manual
                        </span>
                      )}
                    </td>
                    <td className="px-5 py-3.5 text-slate-600">
                      <span className="inline-flex items-center gap-1">
                        <FileText className="h-3.5 w-3.5 text-slate-400" /> {customer._count.documents}
                      </span>
                    </td>
      ...
      <ConfirmDialog
        ...
        description={
          pendingDelete
            ? `${pendingDelete.fullName} and all ${pendingDelete._count.documents} attached document(s) will be permanently removed. This cannot be undone.`
            : ""
        }
        confirmLabel="Delete"
        destructive
