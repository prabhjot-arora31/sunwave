      ...
      <Container className="relative">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <Link
              href="/subsidy"
              className="inline-flex items-center gap-2 rounded-full bg-sun-700 px-6 py-3 text-sm font-semibold text-white hover:bg-sun-800 transition-colors"
            >
          ...
          <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-2xl">
            <h3 className="text-base font-bold text-sky-950 mb-4">
                ...
                <tbody>
                  {subsidySlabs.map((row) => (
                    <tr key={row.capacity} className="border-b border-slate-100 last:border-0">
                      <td className="py-3 text-slate-700">{row.capacity}</td>
                      <td className="py-3 font-semibold text-leaf-600">{row.subsidy}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
